...
public class LimitLoginAuthenticationProviderTest {
	...
	@InjectMocks
	DaoAuthenticationProvider limLogProvider = new LimitLoginAuthenticationProvider();
	
	private Logger logger = LoggerFactory.getLogger(limLogProvider.getClass());
	...
	public void bindMocks() {
		...
		try {
			MemberModifier.field(limLogProvider.getClass(), "logger").set(
					limLogProvider, logger);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void prepatePasswordEncoder() {
		logger.debug("Preparing mock emulation setup for UserPasswordEncoder");
	...
	public Object[][] expiredGenerator() {
		logger.debug("Altering data for Expired account test");
	...
	public Object[][] badCredentialGenerator() {
		logger.debug("Altering data for Bad credetials test");
		Object[][] tmp = new Object[userList.size()][];
		for (int i = 0; i < tmp.length; i++) {
			User user = userList.get(i);
			user.setAccountNonExpired(1);
	...
	public void testSetUserDetailsService() {
		logger.debug("Setting up the userDetailsService");
		limLogProvider.setUserDetailsService(dserv);
		...
		limLogProvider.authenticate(auth);
